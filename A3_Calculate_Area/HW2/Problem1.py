...
mousePoints = open("mouse.txt","r").read().split()
nRow = int(len(mousePoints) / 2)
pointsArray = np.zeros((nRow,2))

for i in range(nRow):
    pointsArray[i][0] = float(mousePoints[i * 2])
    pointsArray[i][1] = float(mousePoints[i * 2 + 1])
